...
*Date:* 2026-06-30 +
*Related docs:* xref:architecture/Desktop_Backend_Sync_Interaction.adoc[], xref:architecture/adr/ADR-003-collaboration-strategy.adoc[], xref:architecture/adr/ADR-008-authentik-auth-model.adoc[]
...
NOTE: CppWiki must not silently discard either side of a conflict.
...
. desktop starts in local-only mode;
. user signs in and connects to the backend;
. backend returns sync bootstrap for the current principal;
. desktop starts replication using that bootstrap;
. documents for assigned workspaces are materialized into the local database;
. once materialized, those documents remain available offline;
. when network returns, local and remote changes are synchronized;
. if versions diverge, a conflict record is created and resolved explicitly.
...
. strengthen sync readiness semantics in `SyncService` / `DocumentSyncService`;
. remove synthetic remote workspace bootstrapping from desktop bridge/page flow;
. introduce workspace hydration tracking in the sync snapshot/model;
. wire hydration state into `Page` and workspace tree UX;
. validate cold start after local DB deletion;
. validate offline edit then reconnect;
. finish conflict UX on top of the already stored conflict records.
...
. done
. done
. done for current desktop/sync model
. done
. done
. partially done: resolve/dismiss exists, compare/merge surface still pending
...
. restart backend/desktop with the new build;
. logout/login in desktop if issuer/audience/client_id changed;
. verify `/api/v1/sync/config` returns assigned `workspace:<id>` channels;
. verify empty workspace rows are visible in desktop;
. create a document in an assigned workspace;
. check Sync Gateway:
...
. check Couchbase query service if Sync Gateway and bucket disagree;
. inspect desktop logs for `CBLite replicator document pushed` or document-level replication errors.
