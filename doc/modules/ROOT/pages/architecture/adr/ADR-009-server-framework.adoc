...
*Date:* 2026-06-18 +
*Author:* Artem +
*Related ADRs:* xref:architecture/adr/ADR-003-collaboration-strategy.adoc[ADR-003 Collaboration Strategy], xref:architecture/adr/ADR-008-authentik-auth-model.adoc[ADR-008 Authentik Auth Model]
...
|Keep oat++
|Removed to consolidate on a framework with stronger static-config, span/tracing and auth extensibility for upcoming phases.

|Drogon
|Drogon is capable, but userver's task-processor model aligns better with the planned observability and service-component boundaries.

|POCO
|Too large for the current skeleton and less coroutine-native.

|Custom server
|Rebuilding HTTP routing, request context and auth plumbing would introduce unnecessary risk.
|===
