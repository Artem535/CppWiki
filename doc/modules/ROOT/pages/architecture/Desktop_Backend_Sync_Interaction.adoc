...
*Date:* 2026-06-30 +
*Related docs:* xref:architecture/Desktop_Auth_Flow.adoc[], xref:architecture/Server_and_Realtime_Editing_Architecture.adoc[], xref:architecture/Sync_Document_Model_and_Conflict_Contract.adoc[], xref:architecture/adr/ADR-003-collaboration-strategy.adoc[], xref:architecture/adr/ADR-008-authentik-auth-model.adoc[], xref:roadmap/Current_Roadmap.adoc[]
...
. the desktop loads the document from the local repository;
. if backend collaboration is unavailable, the document stays local-only editable;
. if backend collaboration is available, the desktop starts a `view` presence session;
. no lock is acquired yet;
. the editor opens in read-only or non-locked viewing state.

NOTE: This is intentional. Opening a document must not automatically reserve writer ownership for an idle user.
...
NOTE: Current inactivity behavior is document-change-based, not mouse-movement-based. This prevents a user from holding the lock indefinitely without real editing activity.
...
. `AuthSessionManager` tries refresh if possible;
. if refresh succeeds, collaboration can continue;
. if refresh fails, bearer token becomes unavailable;
. protected backend requests begin failing;
. collaboration UI must enter an explicit warning state;
. the user must sign in again to resume collaborative editing.

NOTE: Local document persistence must still remain available.
...
NOTE: Lock loss is not treated as a fatal document failure. It is a collaboration state transition.
...
. the user edits locally while the network is unavailable;
. the local repository still accepts validated snapshots;
. sync remains in a degraded or retrying state;
. once the network returns, replication retries;
. local changes are pushed;
. remote changes are pulled;
. the UI returns to a healthy sync state if no conflict occurs.
...
NOTE: The document model should never silently discard the losing version.
...
. Do not bypass the existing desktop repository abstraction.
. Do not couple replication state directly to `QWebEngine` or BlockNote internals.
. Keep lock/presence requests in `BackendClient`, not in the editor bridge.
. Keep auth lifecycle in `AuthSessionManager`, not in sync code.
. Introduce sync status as its own surface in the UI instead of overloading collaboration state.
. Treat replication success and collaboration success as separate signals.
. Preserve local save reliability even if remote sync is failing.
...
. document the current control-plane and data-plane split;
. lock down lease/expiry semantics for locks;
. define offline reconnect behavior;
. define conflict preservation and UI surfacing;
. implement durable sync state reporting in the desktop shell;
. add tests for auth failure, lock loss, lease expiry and reconnect;
. only then move on to broader sync expansion.
