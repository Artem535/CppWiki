...
. генерируется `code_verifier`;
. генерируется `state`;
. считается `code_challenge` по PKCE (`S256`);
. собирается authorize URL;
. system browser открывает Authentik login flow.
...
. реальные protected desktop actions.
...
NOTE: Текущая реализация уже достаточна, чтобы считать `Phase 6` закрытой на dev-стенде. Это ещё не окончательная долгосрочная authentication subsystem, но это уже не просто skeleton.
...
. использовать authenticated identity в collaboration flows вроде lock ownership и presence;
. сделать backend lock ownership авторитативным для write-доступа редактора;
. ввести read-only fallback в desktop editor, если lock принадлежит другому пользователю;
. только после этого переходить к authenticated replication и sync behavior.
