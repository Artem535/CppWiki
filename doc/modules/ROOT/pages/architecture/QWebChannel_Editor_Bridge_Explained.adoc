...
. JS asks C++ for the initial document.
. JS sends changed BlockNote snapshots back to C++.
...
NOTE: The JavaScript editor does not get direct access to files, tokens, database, sync, auth or permissions. It can only call methods explicitly exposed by `QEditorBridge`.
