...
. `state` is generated;
. `code_challenge` is computed using PKCE (`S256`);
. the authorize URL is assembled;
. the system browser is opened.
...
. real protected desktop actions.
...
NOTE: The current implementation is sufficient to consider Phase 6 complete on the development setup. It is still not the final long-term auth subsystem, but it is no longer only a skeleton.
...
. use authenticated identity in collaboration flows such as lock ownership and presence;
. make backend lock ownership authoritative for editor write access;
. enforce read-only fallback in the desktop editor when another user owns the lock;
. then move on to authenticated replication and sync behavior.
