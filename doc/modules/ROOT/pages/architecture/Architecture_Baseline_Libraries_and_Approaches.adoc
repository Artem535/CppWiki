...
. The product is desktop-first and offline-first.
. The canonical document source of truth is structured block JSON, not Markdown, Djot, HTML or Confluence format.
. The editor UX is web-based inside a native Qt shell.
. The C++ core owns persistence, sync, rendering, plugin execution, auth integration and business rules.
. JavaScript editor code must not become the authority for storage, permissions, sync or export.
. Plugins are untrusted and must run inside a WASM sandbox.
. MVP uses Single Writer / Multi Reader locking; realtime collaboration is deferred but must remain architecturally possible.
. AI/LLM capabilities are out of MVP scope and must not be required for core wiki workflows.
...
NOTE: Tokens must never be stored in plain text config files.
...
. impact review against PRD goals and MVP slice;
. migration notes if implementation has already started;
. test plan update for affected contracts.
