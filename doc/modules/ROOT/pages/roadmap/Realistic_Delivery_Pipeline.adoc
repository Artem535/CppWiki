...
*Date:* 2026-06-16 +
*Related docs:* `doc/PRD_v9_Block_Document_Edition.md`, xref:roadmap/Current_Roadmap.adoc[], xref:architecture/Architecture_Baseline_Libraries_and_Approaches.adoc[]
...
. Keep the MVP offline-first and deterministic.
. Treat Block Document JSON as the only source of truth.
. Treat Qt as the application foundation and owner of the product shell.
. Build the C++/TypeScript boundary before building backend features.
. Add external systems only after local save/load is proven.
. Every phase must leave the app runnable.
. Every risky integration gets a spike and an exit gate.
. Do not start realtime collaboration until stable IDs, document validation and persistence exist.
...
|UI shell polish
|Phase 1
|Qt-owned shell only; must not fake unavailable sync/auth states as working

|Backend skeleton
|Phase 5
|Keep desktop local editing independent

|Auth spike
|Phase 6
|Keep tokens out of editor runtime

|RenderService
|Phase 2
|Can start once DTO shape is stable

|Confluence fixtures
|Phase 9
|Needs canonical AST/rendering conventions

|Plugin runtime
|Phase 9
|Safer after rendering boundary exists

|Sync spike
|Phase 8
|Needs auth path and local persistence
|===

== 17. Recommended Immediate Backlog

. Define minimal document DTO with `schema_version`, page ID and block IDs.
. Add fixture tests for valid and invalid starter documents.
. Route `wiki.documents.updateSnapshot` through document validation.
. Add local repository interface before implementing real persistence.
. Keep Couchbase Lite behind the storage boundary and build option.
. Move product navigation/status out of the frontend demo when the Qt shell phase starts.
...
|Defer until Phase 11; do not block editor/document/persistence work

|Couchbase Lite C++ packaging
|C++ wrapper smoke test exists behind `CPPWIKI_ENABLE_CBLITE_STORAGE`; keep packaging optional until Phase 3 persistence work

|Editor bundle packaging
|Keep filesystem `dist` during spikes; decide Qt resources vs installed app data before release hardening

|QWebChannel schema
|Phase 1 baseline closed; generated DTOs continue in Phase 2

|Frontend package manager
|Keep npm unless workspace tooling becomes a real pain point

|JS widget boundary
|Default to Qt; use JS only for BlockNote-native widgets and editor internals
|===
