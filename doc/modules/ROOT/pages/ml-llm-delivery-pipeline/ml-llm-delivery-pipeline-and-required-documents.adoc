////
tags: process, delivery, ml, llm, architecture, methods
date: 2026-03-24
status: Draft
////
...
. Goal and Business Context (Цель и бизнес-контекст)
. Problem Statement (Формулировка проблемы)
. Users and Use Cases (Пользователи и сценарии использования)
. Critical Journeys and Failure-Sensitive Paths (Критические сценарии и рискованные пути)
. Scope and Non-Goals (Границы и что не входит)
. Success Criteria and KPI (Критерии успеха и KPI)
. Deterministic vs ML/LLM Behavior (Детерминированное и вероятностное поведение)
. Requirements Summary (Сводка требований)
. Data, Integrations, Assumptions and Constraints (Данные, интеграции, допущения и ограничения)
. Model / Vendor Strategy (Стратегия модели / вендора)
. Dependencies and Stakeholders (Зависимости и стейкхолдеры)
. Pilot / Release Acceptance (Критерии приемки пилота / релиза)
...
. Context and Scope (Контекст и границы)
. Requirements Summary (Сводка требований)
. Decision Summary and Main Trade-offs (Сводка решения и ключевые компромиссы)
. Workload and Operating Model (Нагрузка и модель эксплуатации)
. Architecture Overview (Обзор архитектуры)
. Components and Integrations (Компоненты и интеграции)
. Key System Scenarios (Ключевые системные сценарии)
. Data Flow (Потоки данных)
. LLM / AI Design (LLM / AI дизайн)
. Reliability and Degradation (Надежность и деградационные режимы)
. Open Questions and Review Notes (Открытые вопросы и заметки ревью)
. Operations Summary (Сводка по эксплуатации)
...
. Chosen Architecture Slice (Архитектурный срез выбранного решения)
. Consequences (Последствия)
. Risks and Mitigations (Риски и митигации)
. Links to Impacted Documents (Связанные и затронутые документы)
. Lifecycle (Жизненный цикл решения)
...
. Purpose and Service Boundary (Назначение и границы сервиса)
. Consumers and Dependencies (Потребители и зависимости)
. API / Tool Methods (Методы API / инструментов)
. Input Schema (Входная схема)
. Output Schema (Выходная схема)
. Data Contract Rules (Правила data contract)
. Prompting and Source Hierarchy (Промпты и иерархия источников)
. Error Handling and Retries (Обработка ошибок и повторы)
. Confidence / Routing Rules (Правила confidence и маршрутизации)
. Edge Cases and Fallback Behavior (Краевые случаи и fallback-поведение)
...
. Evaluation Goal (Цель оценки)
. Quality Dimensions (Измерения качества)
. Acceptance Criteria (Критерии приемки)
. Dataset Strategy (Стратегия датасетов)
. Offline Evaluation (Оффлайн-оценка)
. Manual Review (Ручной review)
. Thresholds and Decision Rule (Пороги и правило принятия решения)
. Regression Policy (Политика регрессии)
. Post-Release Quality Review (Пересмотр качества после релиза)
...
. Delivery Scope (Объем поставки)
. Workstreams and Owners (Потоки работ и владельцы)
. Dependency Map (Карта зависимостей)
. Rollout Plan (План rollout)
. Risks and Blockers (Риски и блокеры)
. Readiness Checklist (Чеклист готовности)
. Migration / Transition Notes (Заметки по миграции / переходу)
...
. Operational Overview (Эксплуатационный обзор)
. Ownership and Support Model (Модель владения и поддержки)
. Key Signals and Alerts (Ключевые сигналы и алерты)
. Common Failure Modes (Типовые сценарии отказов)
. Incident Response (Реакция на инциденты)
. Degradation Modes (Режимы деградации)
. Recovery and Validation (Восстановление и проверка)
. Escalation Contacts (Контакты для эскалации)
...
. Data Sensitivity (Чувствительность данных)
. Storage and Retention (Хранение и retention)
. Access Model (Модель доступа)
. Logging Restrictions (Ограничения на логирование)
. Vendor and Compliance Constraints (Ограничения по вендорам и compliance)
...
. Prompt Purpose and Scope (Назначение и границы промпта)
. System Prompt (Системный промпт)
. Input Variables and Context Blocks (Переменные и блоки контекста)
. Output Contract and Schema Expectations (Ожидаемый output contract и schema)
. Guardrails and Refusal Rules (Ограничения и правила отказа)
. Recommended User Prompt Template (Рекомендуемый user prompt template)
. Examples (Примеры)
. Versioning and Change Notes (Версионирование и история изменений)
...
. `01-prd.md`
. `02-system-design.md`
. `03-adr-001-*.md` и другие ADR по мере необходимости
. `04-service-spec.md`
...
. Сначала зафиксировать `PRD`.
. Затем подготовить `System Design Doc`.
. После этого выделить отдельные `ADR` по спорным архитектурным решениям.
. Затем зафиксировать `Service Spec`.
. Параллельно с проектированием оформить `Evaluation Plan`.
. Перед интеграцией и rollout подготовить `Delivery Plan`.
. Для LLM-сценариев зафиксировать `Prompt Template` до production readiness.
. Перед production readiness подготовить `Runbook` и `Security Note`.
