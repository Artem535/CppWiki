...
### 1.1 Discovery-First Rule

Перед `System Design Doc`, `ADR` и выбором модели команда должна пройти **delivery discovery gate**. На этом шаге задача формулируется как поставка проверяемого продукта или сервиса, а не как абстрактная ML/LLM-инициатива.

Discovery считается достаточным, если зафиксированы:

- пользовательский outcome и решение/действие, которое сервис должен поддерживать;
- целевые пользователи, операционный контекст и критические user journeys;
- scope, non-goals и минимальный shippable slice для пилота;
- измеримые acceptance criteria до выбора архитектуры;
- разделение deterministic product behavior и probabilistic ML/LLM behavior;
- данные, интеграции, privacy/compliance и human-review assumptions;
- release/evaluation gates, которые позволяют принять решение `go / no-go`.

Если эти пункты не заполнены, проект не должен переходить к архитектурному дизайну как к "готовой задаче". Допускается technical spike, но он должен быть оформлен как discovery activity с явными open questions.
...
- какой минимальный пилотный slice можно проверить offline и в ограниченном rollout;
- где заканчивается deterministic product behavior и начинается probabilistic ML/LLM behavior.
...
- scope / non-goals;
- success criteria и KPI по product, quality, latency, cost, safety и operations;
- deterministic vs ML/LLM behavior split;
- данные, интеграции, ограничения и допущения;
- модельная / vendor strategy на уровне trade-off, без детального system design;
...
- понятны целевые пользователи, user journeys и failure-sensitive paths;
- критерии успеха измеримы до выбора архитектуры;
- зафиксирован smallest shippable slice для MVP/пилота;
- понятно, почему нужен именно ML/LLM-подход, а не rule-based automation;
- явно разделены deterministic requirements и probabilistic ML/LLM behavior;
- зафиксированы data, integration, privacy, cost, latency и human-review assumptions;
- есть release/evaluation gates для решения `go / no-go`.

**Stage gate:**

Команда может переходить к `Stage 2. System Design`, только если PRD отвечает на discovery-вопросы из раздела `1.1`. Если часть ответов неизвестна, они должны быть оформлены как open questions с владельцем и дедлайном.
...
1. **PRD / Service PRD**
   Объясняет, зачем существует сервис, какую проблему он решает, какие у него scope, KPI, non-goals, user journeys, failure-sensitive paths, deterministic/probabilistic split и ограничения.
2. **System Design Doc**
...
Без этих девяти артефактов handoff считается базово неполным, а для LLM-сервиса полноценный handoff почти всегда требует `09 Prompt Template`.
...
| Prompt Template   | DS Lead                                 | Team Lead, QA, Архитектор                                  | Разработчики, QA, Delivery Team                        |
...
- [ ] Зафиксирован `PRD` с понятными problem statement, scope, non-goals, user journeys и failure-sensitive paths.
- [ ] Success criteria измеримы по product, quality, latency, cost, safety и operations.
- [ ] Deterministic behavior отделен от probabilistic ML/LLM behavior.
- [ ] Определен smallest shippable slice для offline evaluation и пилота.
- [ ] Data, integration, privacy, vendor, cost и human-review assumptions явно записаны.
