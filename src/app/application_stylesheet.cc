...
auto ResolveApplicationStylesheetPath() -> QString {
  // A Qt resource path (":/...", see constants::kApplicationQssPath) is embedded in the binary
  // itself, so it resolves the same way regardless of CWD or install layout -- no
  // candidate-path guessing needed (see issue #182 / #152's identical fix for
  // kQlementineDarkThemePath).
  const auto path = ToQString(constants::kApplicationQssPath);
